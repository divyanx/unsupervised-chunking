...
def clust(a, size, k):
    ...
    for j in range(0, size):
        ...
        a[m][j] = a[j][m]
        a[j][n] = 99999999999999999999.0
        a[n][j] = 99999999999999999999.0
    return a
...
def augmented_dendrogram(*args, **kwargs):
    ...
    if not kwargs.get('no_plot', False):
        for i, d in zip(data['icoord'], data['dcoord']):
            ...
            y = d[1]
            plt.plot(x, y, 'ro')
            plt.annotate("%.3g" % y, (x, y), xytext=(0, 12),
...
plt.ylabel('Distance')
augmented_dendrogram(Z, labels=names, show_leaf_counts=True,
                     p=25, truncate_mode='lastp')
plt.show()
